# 📝 Digital Literacy Portfolio - CSE0001

## 📌 Project Overview
This repository contains my **Digital Literacy Portfolio** for the CSE0001 course. Developed under the **Student Digital Ambassador** scenario, it includes five distinct tasks—one for each course module. These tasks demonstrate proficiency in digital tools, maintaining a professional online presence, utilizing coding platforms, practicing email etiquette, and understanding cybercrime awareness.

---

## 📂 Module Summaries

### Task 1: Digital Literacy Awareness Infographic (Module 1)
* **Description:** A one-page infographic created using [Canva](https://www.canva.com/) covering essential topics such as digital literacy, safe internet practices, and useful digital tools for students.
* **Directory:** `task-1-presentation/`

### Task 2: Student Digital Portfolio (Module 2)
* **Description:** Setup and curation of professional profiles across key industry platforms to build a strong online presence.
* **Directory:** `task-2-portfolio/`

| Platform | Profile Link |
| :--- | :--- |
| **GitHub** | https://github.com/dewanganvaibhav256-ops | 
| **LinkedIn** | https://www.linkedin.com/in/vaibhav-dewangan-8896163a2/ |
| **Kaggle** | https://www.kaggle.com/vaibhavdewangan05 |

### Task 3: Coding & Collaboration Platforms (Module 3)
* **Description:** * **Coding Practice:** Completed a beginner challenge on [HackerRank](https://www.hackerrank.com/).
  * **Google Form:** Created a 5-question Digital Literacy Awareness Quiz.
* **Quiz Link:** [Take the Quiz Here](https://docs.google.com/forms/d/e/1FAIpQLSenxERlSz1jngjrCr-NWIRfXBNrUrKmJaEAMBtOZbNXHK7CAA/viewform?usp=sharing&ouid=103660322356989056092)
* **Directory:** `task-3-platforms/`

### Task 4: Professional Email & Etiquette Guide (Module 4)
* **Description:** * Drafted two professional emails: an assignment extension request and an internship inquiry.
  * Created a comprehensive *Social Media Do's and Don'ts* checklist.
* **Directory:** `task-4-email-etiquette/`

### Task 5: Cybercrime Awareness Case Study (Module 5)
* **Description:** A 200–250 word case study focusing on **UPI Fraud**.
* **Reporting Resources:** * **Portal:** [cybercrime.gov.in](https://cybercrime.gov.in/)
  * **National Helpline:** 1930
* **Directory:** `task-5-cybercrime/`

---
> *This repository was created by Vaibhav Dewangan as part of the CSE0001 coursework.*

